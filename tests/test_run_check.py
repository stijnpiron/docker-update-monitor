...
class TestRunCheckContainerProcessing:
    ...
    @patch("monitor.fetch_all_tags", return_value=["1.1.0"])
    @patch("monitor.get_dockerhub_token", return_value="token")
    @patch("monitor.docker")
    def test_container_without_image_tags_uses_fallback(self, mock_docker, mock_token, mock_fetch):
        ...
        with patch.object(monitor, "GITHUB_TOKEN", ""):
            monitor.run_check()
    ...
    def test_container_with_no_image_ref_skipped(self, mock_docker, mock_token, caplog):
        ...
        with patch.object(monitor, "GITHUB_TOKEN", ""), caplog.at_level(logging.WARNING):
            monitor.run_check()

        assert "Cannot determine image reference" in caplog.text

    @patch("monitor.fetch_all_tags", return_value=[])
    @patch("monitor.get_dockerhub_token", return_value="token")
    @patch("monitor.docker")
    ...
    @patch("monitor.fetch_all_tags", return_value=["1.0.0"])
    @patch("monitor.get_dockerhub_token", return_value="token")
    @patch("monitor.docker")
    def test_no_updates_found_logs_info(self, mock_docker, mock_token, mock_fetch, caplog):
        ...
        with patch.object(monitor, "GITHUB_TOKEN", ""), caplog.at_level(logging.INFO):
            monitor.run_check()

        assert "No updates found" in caplog.text

    @patch("monitor.fetch_all_tags", return_value=["1.0.0", "2.0.0"])
    ...
    def test_github_token_present_logs_info(self, mock_docker, mock_token, mock_fetch, caplog):
        ...
        with patch.object(monitor, "GITHUB_TOKEN", "gh_token"), caplog.at_level(logging.INFO):
            monitor.run_check()
...
class TestMainEdgeCases:
    ...
    def test_invalid_cron_exits(self):
        with patch.object(monitor, "CRON_SCHEDULE", "invalid cron"), \
    ...
    def test_dry_run_logs_mode(self, mock_run_check, caplog):
        ...
        with patch.object(monitor, "DRY_RUN", True), \
             patch.object(monitor, "CRON_SCHEDULE", "0 * * * *"), \
             patch.object(monitor, "RUN_ON_STARTUP", False), \
